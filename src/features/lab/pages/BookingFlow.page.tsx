...
import { NestJsBookingFlowLabAccordion } from "@/features/lab/components/NestJsBookingFlowLabAccordion";
import {
  NESTJS_BOOKING_FLOW_COMPLETION_POINTS,
  NESTJS_BOOKING_FLOW_EXPECTED_LEARNING,
  NESTJS_BOOKING_FLOW_FEATURE_SEQUENCE,
  NESTJS_BOOKING_FLOW_LAB_SECTIONS,
  NESTJS_BOOKING_FLOW_OVERVIEW_CARDS,
  NESTJS_BOOKING_FLOW_STAGES,
} from "@/features/lab/constants/booking-flow-content.constant";
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-6">
      <SetupGuideHeader
        badge="Lab"
        title="Booking flow"
        description="ฝึก Git workflow แบบครบลูปบนโปรเจกต์ NestJS booking app ตั้งแต่สร้าง app, แตกหลาย branch, generate resources, push PR, merge และ rebase จนครบทุก feature"
      />

      <section className="rounded-2xl border border-border bg-card p-4 shadow-sm md:p-6">
        <h2 className="text-xl font-black tracking-tight text-foreground">Overview</h2>
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          เริ่มจาก baseline ที่นิ่งบน main ก่อน แล้วค่อยแตก 10 feature branches เพื่อให้ทุกกิ่งของ
          booking app ต้องกลับมา rebase ตาม main ระหว่างทาง
        </p>

        <div className="mt-4 grid gap-4 xl:grid-cols-2">
          {NESTJS_BOOKING_FLOW_OVERVIEW_CARDS.map((card) => (
      ...
      <section className="rounded-2xl border border-border bg-card p-4 shadow-sm md:p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          ...
          main ได้แบบควบคุมได้ แม้จะเปลี่ยนเป็นโดเมน booking
        </p>

        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {NESTJS_BOOKING_FLOW_STAGES.map((stage, index) => (
      ...
      <section className="rounded-2xl border border-border bg-card p-4 shadow-sm md:p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          ใช้ลำดับนี้เป็นคิว merge หลักของ lab เพื่อให้ branch หลัง ๆ ต้อง sync ตาม main ล่าสุดจริงใน
          booking workflow
        </p>

        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {NESTJS_BOOKING_FLOW_FEATURE_SEQUENCE.map((feature) => (
      ...
      <section className="rounded-2xl border border-border bg-card p-4 shadow-sm md:p-6">
        ...
        <NestJsBookingFlowLabAccordion
          labSections={NESTJS_BOOKING_FLOW_LAB_SECTIONS}
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <article className="rounded-2xl border border-border bg-card p-4 shadow-sm md:p-6">
          ...
          <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-muted-foreground">
            {NESTJS_BOOKING_FLOW_COMPLETION_POINTS.map((point) => (
        ...
        <article className="rounded-2xl border border-border bg-card p-4 shadow-sm md:p-6">
          ...
          <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-muted-foreground">
            {NESTJS_BOOKING_FLOW_EXPECTED_LEARNING.map((point) => (
              <li key={point}>{point}</li>
            ))}
          </ul>
        </article>
