...
import type {
  NestJsBookingFlowLabDifficulty,
  NestJsBookingFlowLabSection,
} from "@/features/lab/constants/booking-flow-content.constant";

type NestJsBookingFlowLabAccordionProps = {
  labSections: NestJsBookingFlowLabSection[];
  copyStatusByCommand: Record<string, CommandCopyStatus | undefined>;
  onCopyCommand: (commandKey: string, command: string) => Promise<void>;
};

const DIFFICULTY_CLASS_NAME: Record<NestJsBookingFlowLabDifficulty, string> = {
...
export function NestJsBookingFlowLabAccordion({
  labSections,
  copyStatusByCommand,
  onCopyCommand,
}: NestJsBookingFlowLabAccordionProps) {
  return (
                ...
                <AccordionContent>
                  <div className="flex flex-col gap-4 pb-2">
                    ...
                    <div className="rounded-xl border border-border bg-background/80 p-4">
                      ...
                      {lab.rebaseCommands.length ? (
                        <div className="mt-3 flex flex-col gap-3">
                          {lab.rebaseCommands.map((commandItem, commandIndex) => {
                            const commandKey = `lab-${lab.id}-rebase-${commandIndex}`;
                            const status = copyStatusByCommand[commandKey];
