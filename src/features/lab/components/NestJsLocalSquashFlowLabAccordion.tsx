...
import type {
  NestJsLocalSquashFlowLabDifficulty,
  NestJsLocalSquashFlowLabSection,
} from "@/features/lab/constants/local-squash-flow-content.constant";

type NestJsLocalSquashFlowLabAccordionProps = {
  labSections: NestJsLocalSquashFlowLabSection[];
  copyStatusByCommand: Record<string, CommandCopyStatus | undefined>;
  onCopyCommand: (commandKey: string, command: string) => Promise<void>;
};

const DIFFICULTY_CLASS_NAME: Record<NestJsLocalSquashFlowLabDifficulty, string> = {
...
export function NestJsLocalSquashFlowLabAccordion({
  labSections,
  copyStatusByCommand,
  onCopyCommand,
}: NestJsLocalSquashFlowLabAccordionProps) {
  return (
                  ...
                  <div className="flex flex-col gap-4 pb-2">
                    ...
                    <div className="rounded-xl border border-border bg-background/80 p-4">
                      ...
                      <div className="mt-3 flex flex-col gap-3">
                        {lab.commands.map((commandItem, commandIndex) => {
                          const commandKey = `lab-${lab.id}-command-${commandIndex}`;
                    ...
                    <div className="rounded-xl border border-dashed border-border bg-background/80 p-4">
                      ...
                      <ul className="mt-2 flex list-disc flex-col gap-1 pl-5 text-sm leading-6 text-muted-foreground">
                        {lab.generatorChoices.map((choice) => (
                    ...
                    <div className="rounded-xl border border-border bg-background/80 p-4">
                      ...
                      <div className="mt-3 flex flex-col gap-3">
                        {lab.squashCommands.map((commandItem, commandIndex) => {
                          const commandKey = `lab-${lab.id}-squash-${commandIndex}`;
                          const status = copyStatusByCommand[commandKey];
