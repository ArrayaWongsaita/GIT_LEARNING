...
export function GitRebaseLabAccordion({
  copyStatusByCommand,
  onCopyCommand,
}: GitRebaseLabAccordionProps) {
  return (
    <div className="mt-5 flex flex-col gap-5">
      {GIT_REBASE_LAB_SECTIONS.map((section) => (
        <section
          key={section.id}
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="mt-2 flex flex-col gap-2">
              <div className="flex flex-wrap gap-2">
                <span className="inline-flex rounded-full border border-primary/30 bg-primary/10 px-2.5 py-1 text-[11px] font-semibold tracking-wide text-primary">
                  Sandbox: {section.workspace}
                </span>
              </div>
              <p className="text-sm leading-6 text-foreground">
                <span className="font-semibold text-foreground">Focus files:</span>{" "}
                {section.focusFiles.join(", ")}
              </p>
            </div>
          </div>
