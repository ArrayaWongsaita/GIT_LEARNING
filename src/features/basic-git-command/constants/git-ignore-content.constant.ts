...
export type GitIgnoreSectionItem = {
  ...
  title: string;
  purpose: string;
  commands: GitIgnoreCommand[];
  notes?: string[];
};

export type GitIgnoreSection = {
  id: string;
  title: string;
  summary: string;
  items: GitIgnoreSectionItem[];
};

export type GitIgnoreLab = {
  id: string;
  title: string;
  summary: string;
...
export type GitIgnoreLabSection = {
  id: string;
  title: string;
  summary: string;
  labs: GitIgnoreLab[];
...
export const GIT_IGNORE_SECTIONS: GitIgnoreSection[] = [
  ...
  {
    id: "handle-tracked-files",
    title: "แก้กรณีไฟล์ถูก track ไปแล้ว",
    summary: ".gitignore จะไม่หยุดติดตามไฟล์ที่เคยเข้า index มาก่อน ต้องลบออกจาก cache ก่อน",
    items: [
      {
        id: "remove-from-index",
        title: "หยุด track ไฟล์หรือโฟลเดอร์ที่เคย add ไปแล้ว",
        purpose:
          "เอาไฟล์ออกจาก staging/index โดยไม่ลบไฟล์จริงในเครื่อง เพื่อให้ .gitignore เริ่มมีผล",
        commands: [
          {
            command: "git rm --cached .env",
            description:
              "ลบ .env ออกจาก index แต่ยังเก็บไฟล์ไว้ในเครื่องสำหรับใช้งานต่อ",
          },
          {
            command: "git rm -r --cached dist",
            description:
              "ลบทั้งโฟลเดอร์ dist ออกจากการ track เพื่อให้ build output ไม่เข้า commit ถัดไป",
          },
          {
            command: "git status",
            description: "ตรวจผลว่าไฟล์ถูก mark เป็น deleted from index และพร้อม commit การเปลี่ยนแปลง",
          },
        ],
        notes: [
          "หลังรัน git rm --cached แล้วควร commit เพื่ออัปเดต history ให้ตรงกับกฎใหม่",
        ],
      },
      {
        id: "local-only-rules",
        title: "ignore เฉพาะเครื่องตัวเองโดยไม่ commit rule",
        purpose:
          "ใช้กับไฟล์ local notes หรือ config เฉพาะเครื่องที่ไม่ควรบังคับทุกคนในทีม",
        commands: [
          {
            command: "printf \"notes.txt\\n\" >> .git/info/exclude",
            description:
              "เพิ่มกฎ ignore แบบ local-only ซึ่งจะไม่ถูก commit ขึ้น repository",
          },
          {
            command: "git check-ignore -v notes.txt",
            description:
              "เช็กว่าไฟล์ local ถูก ignore จาก .git/info/exclude เรียบร้อยแล้ว",
          },
        ],
      },
    ],
...
export const GIT_IGNORE_LAB_SECTIONS: GitIgnoreLabSection[] = [
  {
    id: "gitignore-lab",
        ...
        commands: [
          {
            ...
            description: "สร้างโฟลเดอร์ทดลองสำหรับฝึก .gitignore",
          },
          {
            command: "cd gitignore-lab",
